...
binary_task_mri_image_path = "MRI_Image_Matrix.binary.npy"  # The path of dataset - image
mri_image_matrix_binary_task = np.load(binary_task_mri_image_path)  # The operation to load the

X = np.delete(mri_image_matrix_binary_task, 262144, 1)  # The process to delete the last column - labels
Y = mri_image_matrix_binary_task[:, -1]  # The process to obtain the labels
...
model.compile(loss=tf.losses.binary_crossentropy,
              ...
              metrics=['accuracy'])  # Task A Binary task

# Task B Multiclass task  model.compile(loss=tf.losses.categorical_crossentropy,
#                         optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001),
#                         metrics=['accuracy'])
